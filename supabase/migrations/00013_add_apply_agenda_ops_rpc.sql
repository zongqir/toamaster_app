-- Agenda V2 atomic ops apply RPC (item-level patch + optimistic lock)

CREATE OR REPLACE FUNCTION public.assert_segment_budget_v2(
  target_meeting_id TEXT,
  target_parent_item_key TEXT
) RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF target_parent_item_key IS NULL THEN
    RETURN;
  END IF;

  SELECT budget_limit_seconds
  INTO v_budget_limit
  FROM agenda_items_v2
  WHERE meeting_id = target_meeting_id
    AND item_key = target_parent_item_key
    AND node_kind = 'segment'
    AND budget_mode = 'hard_cap'
  ...
  SELECT COALESCE(SUM(planned_duration), 0)
  INTO v_total_planned
  FROM agenda_items_v2
  WHERE meeting_id = target_meeting_id
    AND parent_item_key = target_parent_item_key
    AND consume_parent_budget = TRUE
    AND deleted_at IS NULL;

  IF v_total_planned > v_budget_limit THEN
    RAISE EXCEPTION 'PARENT_BUDGET_EXCEEDED:%/%', v_total_planned, v_budget_limit;
  END IF;
END;
$$;
...
BEGIN
  ...
  IF NOT public.has_meeting_role_v2(
    p_meeting_id,
